...
for i in range(3):
    ...
    for train_times in range(fold_num):
        '''
        构建每个fold的val时的正负样本构成的矩阵
        去掉了人为构建的DTI-GAT模型中的adj数据
        test_dti_inter_mat获得的是包括train、test的所有的val的正负样本
        '''
        test_dti_inter_mat = -np.ones((drug_num, protein_num))  # [protein_num, drug_num]
        for j, inter in enumerate(test_train_interact_pos[train_times]):  # 构建train邻接矩阵
            protein_id = inter[1]
            drug_id = inter[0]
            label = test_train_label[train_times][j]
            test_dti_inter_mat[drug_id][protein_id] = label  # positive sample为1，negative sample为0，其余为-1
        for j, inter in enumerate(test_val_interact_pos[train_times]):  # 构建test集邻接矩阵
            protein_id = inter[1]
            drug_id = inter[0]
        ...
        load encoder data
        """
        nei_index, feats, mps, poss, sc_edge, mp_edge = load_data(args.dataset, train_times)  # 在数据预处理中mps, pos是已经计算好的DAD
        ...
        for epoch in range(args.epochs):
            ...
            '''
            在每个epoch中分别生成每个epoch的负样本
            源程序为每次生成5个fold的负样本，我们每次只获取当前fold的负样本
            实现根据预测结果进行负样本
            '''
