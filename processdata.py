...
def add_dti_info(protein_num, drug_num, ori_dti_inter_mat, positive_sample_num, train_positive_inter_pos, val_positive_inter_pos, refer_val_interact_pos,
    ...
    '''
    因为每个epoch只生成了一个fold的负样本，所以并不需要KFold的分折操作，
    只需要按照给定的正样本fold将负样本划分为train、test集，并构建dti矩阵
    '''
    ...
    train_negative_inter_pos.append(negative_interact_pos[:len(train_positive_inter_pos) * rate_n])
    val_negative_inter_pos.append(negative_interact_pos[len(train_positive_inter_pos) * rate_n:])
    ...
    for i in range(1):
        ...
        train_negative_label = np.zeros(len(train_negative_inter_pos[0]) * rate_n)
        ...
        val_negative_label = np.zeros(len(val_negative_inter_pos[0]) * rate_n)
...
def first_spilt_label(inter, groups, seed, dataset,pos_num):
    ...
    for i in range(5): # 数据集总共分成5组，总共5个test集，每次一个分组
        val_fold_id = i
        for j in range(5):  # 当前数据分组作为test集
            if j != val_fold_id:
    ...
    # parent_dir = os.path.join(root_dir, 'encoder_' + str(0))
